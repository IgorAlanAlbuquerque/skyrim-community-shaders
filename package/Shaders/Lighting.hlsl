...
#		if defined(DEFERRED)
	// RG: duv for SSDM pyramid + solve; B: forward coverage (clip-stable + |duv| clamp), A unused. Cleared to 0 before the pass.
	float4 ssdmPack = float4(0, 0, 0, 0);
#			if defined(EMAT)
	if (ssdmActive) {
		float ssdmForwardCov;
		float2 duv = ExtendedMaterials::ComputeDisplacementVector(
			viewPosition, ssdmViewDir, tbnTr[0], tbnTr[1], tbnTr[2], ssdmHeight, ssdmDispScale, eyeIndex, ssdmForwardCov);
		ssdmPack = float4(duv, ssdmForwardCov, 0.0);
	}
#			endif
